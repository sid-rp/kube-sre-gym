...
WARMUP_SCENARIOS = [
    ...
    },
    {
        ...
        "steps": [{"action": "kubectl patch deployment/payment-api -n payments -p '{\"spec\":{\"template\":{\"spec\":{\"containers\":[{\"name\":\"payment-api\",\"command\":[\"sh\",\"-c\",\"exit 1\"]}]}}}}'",
                    "effect": "payment-api crashes on startup with exit code 1"}],
